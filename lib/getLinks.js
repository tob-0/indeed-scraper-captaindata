...
module.exports = {
     ...
     */
    getLinks: async (q,l) =>{
        const browser = await puppeteer.launch({headless: false,slowMo:0}) // For debugging, allows to see what's happening + Slow motion (delay by 250ms)
        ...
        await page.waitForSelector('div.jobsearch-SerpJobCard').catch(err=>{
            console.log('An error occured while waiting for job cards (Captcha ?): '+err.message)
            exit(0)
        })
